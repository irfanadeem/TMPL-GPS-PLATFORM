...
class ReportDashboardCommand extends Command
{
     ...
     * @var string
     */
    protected $signature = 'command:ReportDashboard 
                            {--start-time= : Starting time for calculation (Y-m-d H:i:s format) (default: today) optional}
    ...
    public function handle()
    {
        ...
        // If no start time provided, use today as default
        if (!$startTime) {
            $startTime = Carbon::today();
            $this->info('No start time provided, using today as default: ' . $startTime->format('Y-m-d H:i:s'));
        } else {
            try {
