...
class FuelLevelChangeCheck extends AlertCheck
{
    ...
    public function checkEvents($position, $prevPosition)
    {
        ...
        if($position->speed > 0) {
        Log::info('Speed is greater than 0', ['device-Id' => $this->device->id, 'speed' => $position->speed]);
            return null;
        }
    ...
    private function checkSensorEvents($sensor, $position, $prevPosition)
    {
        ...
        if ($change['difference'] > 2500) {
        Log::info('Change difference - ', ['device-Id' => $this->device->id, 'difference' => $change['difference']]);
            return $fail();
        }
