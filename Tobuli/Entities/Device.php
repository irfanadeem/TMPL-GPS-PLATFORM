...
class Device extends AbstractEntity implements DisplayInterface, FcmTokenableInterface, ChattableInterface
{
    ...
    {
        if ( ! $this->traccar) {
            $traccar = new TraccarDevice();
            $traccar->id = $this->traccar_device_id ?: 0;
    ...
    {
        return $this->belongsToMany('Tobuli\Entities\CronJobCalculation', 'id', 'device_id');
    }
    ...
    {
        return $this->belongsToMany('Tobuli\Entities\DeviceGroup', 'user_device_pivot', 'device_id', 'group_id');
    }
    ...
    public function getFuelConsum($dateFrom, $dateTo) {
        $first = $this->positions()
        ...
        $fuelSensor = $this->getFuelTankSensor();
        $fuelCounsum=0;
        if ( ! is_null($fuelSensor)) {
           $fuel_fill = $this->last_event()->where('type', 'fuel_fill')->where('time','>=',$dateFrom)->first();
            if (isset($fuel_fill) && $fuel_fill != null) {
                $fuel_fill_value = $fuel_fill->additional['difference']?? 0;
                $fuelCounsum = ($fuelSensor->getValue($positions[1]['other'], false) + $fuel_fill_value) - $fuelSensor->getValue($positions[0]['other'], false);
            }else {
                $fuelCounsum = $fuelSensor->getValue($positions[0]['other'], false) - $fuelSensor->getValue($positions[1]['other'], false);
            }
        }

        return ($fuelCounsum > 0) ? $fuelCounsum : 0;
    }
    public function getFuel($dateFrom) {
        ...
        $fuelSensor = $this->getFuelTankSensor();
        if ( ! is_null($fuelSensor)) {
            $fuelValue = $fuelSensor->getValue($positions[0]['other'], false);
            return $fuelValue<9999?$fuelValue:0;
        }else {
            return 0;
        }
        
    }
    public function getCurrentFuelById($id) {
    ...
    public function getFuelDayEnd($date) {
    ...
    public function getFuelConsumByTrip($dateFrom, $dateTo) {
        $first = $this->positions()
            ->whereBetween('time', [$dateFrom, $dateTo])
            ->orderBy('time', 'asc')
            ->limit(1);
        ...
        $fuelCounsum=0;
        if ( ! is_null($fuelSensor)) {
            $fuel_fill = $this->last_event()->where('type', 'fuel_fill')->whereBetween('time', [$dateFrom, $dateTo])->first();
            if (isset($fuel_fill) && $fuel_fill != null) {
                $fuel_fill_value = $fuel_fill->additional['difference']?? 0;
                $fuelCounsum = ($fuelSensor->getValue($positions[1]['other'], false) + $fuel_fill_value) - $fuelSensor->getValue($positions[0]['other'], false);
            }else {
