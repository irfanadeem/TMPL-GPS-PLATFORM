    ...
    SecondaryCredentialsInterface
{
    ...
    {
        if ($this->isAdmin() || $this->isSupervisor()) {
            return \Tobuli\Entities\Device::query()
                ->select('devices.*', 'user_device_pivot.group_id')
                ->where(function ($query) {
                    $query->where('devices.user_id', $this->id)
    ...
    public function devicesWithGroups($filterData= [])
    {

        return $this->hasMany('Tobuli\Entities\Device', 'user_id', 'id')
            ->select('devices.*', 'user_device_pivot.group_id', 'device_groups.title as group_name')
            ->orWhere(function ($query) {
                $query->whereNull('devices.user_id')->orWhere('devices.user_id', '>', 0);
            })
            ...
            ->Join('device_groups', function ($join) use ($filterData) {
                $join->on('user_device_pivot.group_id', '=', 'device_groups.id');
                if (!empty($filterData['_groups'])) {
                        $join->where('device_groups.id', '=', $filterData['_groups']);
                    }
