...
<style>
   ...
   <div id="EventsLookupTableContainer" class="container">
        <!-- Buttons for Download, Print, Email -->
     
        
        <!-- DataTable with Bootstrap shadow -->
        <div class="shadow-container" style="color: white;background-color: #171821">
               <div class="btn-container" style="margin-bottom: 60px;">
            <h4 style="float: left; margin-right: 5px;" >{{ $events[0]->alert_name??''; }}</h4>
        </div>
                    ...
                    @foreach($events as $index => $row) 
                                    <tr>
                                        ...
                                        @if(is_array($row->additional))
                                            @foreach($row->additional as $key => $value)
                                            @if($key == 'difference' && $value < 2500) 
                                        ...
                                        <td>
                                            <a href="https://www.google.com/maps?q=<?= $row->latitude ?>,<?= $row->longitude ?>"
                                               target="_blank">
                                                <img style="height: 40px; background-repeat: no-repeat; pointer-events: none;"
                                                     src="/mapicon.png">
                                            </a>
                                        </td>
                                    </tr>
