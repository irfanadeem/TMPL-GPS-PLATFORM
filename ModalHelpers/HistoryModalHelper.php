...
class HistoryModalHelper extends ModalHelper {
    ...
    public function getFiltered()
    {
        ...
        // Settings
        if (isset($this->data['snap_to_road']) && ($this->data['snap_to_road'] == 'true' || $this->data['snap_to_road'] == 1)) {
            $snapToRoad = true;
        } else {
            $snapToRoad = false;
        }
        
        // Re-use existing getHistoryData logic which sets up DeviceHistory with sensors/actions
        $historyData = $this->getHistoryData($device);
        
        $groups = $historyData['groups'];
        $root = $historyData['root'];
        
        $positions = [];
        $totalDistance = 0;
        
        // Iterate groups to flatten positions
        foreach ($groups->all() as $group) {
            $status = self::STATUS_DRIVE;
            if ($group->getKey() == 'stop') $status = self::STATUS_STOP;
            if ($group->getKey() == 'event') continue; // Skip event markers for now? Or include them? detailed spec says segments. 

            // Logic to determine segment status for API
            // 1 = Driving, 2 = Stopped, 3 = Idle (not explicitly split in basic logic, often idle is part of stop or drive depending on config)
            // For now mapping: Drive -> 1, Stop -> 2.
            
            $segmentStatus = ($status == self::STATUS_STOP) ? 2 : 1;
            $stopDuration = ($status == self::STATUS_STOP) ? $group->getStat('duration')->value() : 0;

            $groupPositions = $group->getStat('positions')->value();
            
            if ($snapToRoad) {
                $groupPositions = snapToRoad($groupPositions);
            }

            foreach ($groupPositions as $pos) {
                // Ensure filtered valid points only (DeviceHistory actions usually handle this, but double check 'valid' if possible)
                // DeviceHistory logic normally includes all points in the segment.
                // We want to pass "clean" data.
                
                // Calculate distance from previous point if not set
                ...
                $positions[] = [
                    ...
                    'speed' => (int)$pos->speed,
                    'time' => $pos->time, // Formatter conversion needed? Spec says Y-m-d H:i:s
                    'distance' => round($distance, 3), // meters or km? Web usually works in km. Spec says "0.125 km". DeviceHistory distance is usually km.
                    'cumulative_distance' => round($totalDistance, 3),
                    'segment_status' => $segmentStatus,
                    'stop_duration' => $stopDuration,
                    'valid' => 1 // Implicitly valid if it made it into the group logic usually
                ];
                
                // Reset stop duration after first point in segment to avoid duplication? 
                // Spect implies strictly point attributes. 
                // "stop_duration" on every point in a stop segment is useful.
            }
        }

        // Stats
        $stats = [
            'driving_time' => $root->getStat('drive_duration')->human(),
            'stopped_time' => $root->getStat('stop_duration')->human(),
            'distance' => $root->getStat('distance')->human(),
            'fuel_consumption' => $root->hasStat('fuel_consumption') ? $root->getStat('fuel_consumption')->human() : null,
        ];
        ...
        return [
            ...
            'total_points' => count($positions),
            'total_distance' => round($totalDistance, 2),
            'duration' => $root->getStat('duration')->human(),
            'positions' => $positions,
            'stats' => $stats
        ];
    }
