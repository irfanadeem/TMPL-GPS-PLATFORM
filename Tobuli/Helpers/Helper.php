...
function analyzeFuelData($positions, $sensor, $fillThreshold = 1, $thriftThreshold = 1) {
    $min = null; // Minimum fuel level
    $max = null; // Maximum fuel level
    $fuelFill = 0; // Total significant fuel filled
    $fuelThrift = 0; // Total significant fuel theft
    $fuelConsumption = 0; // Fuel consumed
    $first = null; // First fuel level
    $last = null; // Last fuel level
    $events = []; // To store details of fills and thefts

    foreach ($positions as $position) {
        $current = $sensor->getValuePosition($position);
        if ($current < 2000) {

            if ($current === null) {
                continue; // Skip positions with no sensor data
            }
            ...
            // Check for significant fuel fill (increase > threshold)
            if ($last !== null && $current - $last > $fillThreshold) {
                $fuelFill += $current - $last; // Add to total fuel fill
                $events[] = [
                    'type' => 'fuel_fill',
                    'amount' => $current - $last,
            ...
            // Check for significant fuel theft (decrease > threshold)
            if ($last !== null && $last - $current > $thriftThreshold) {
                $fuelThrift += $last - $current; // Add to total fuel theft
                $events[] = [
                    'type' => 'fuel_theft',
                    'amount' => $last - $current,
                    'position_id' => $position->id
            ...
            $last = $current; // Update the last value
        }

        // Calculate fuel consumption
        if ($first !== null && $last !== null && $last > $first) {
            $fuelConsumption = ($last - $first) - $fuelThrift + $fuelFill;
        }
    }
    ...
    return [
        'min_fuel_level' => $min,
        'max_fuel_level' => $max,
        'total_fuel_filled' => $fuelFill,
        'total_fuel_theft' => $fuelThrift,
        'fuel_consumption' => $fuelConsumption,
        'events' => $events
    ];
}

function calculateTotalHours($positions) {
    // Initialize totals
    $totalMovingHours = 0;
    $totalIdleHours = 0;

    // Track the previous position's time
    $previousTime = null;

    foreach ($positions as $position) {
        // Load the XML data for the 'other' field
        $xml = simplexml_load_string($position['other']);

        // Extract relevant fields
        $motion = (string) $xml->motion; // 'true' or 'false'
        $ignition = (string) $xml->ignition; // 'true' or 'false'
        $currentTime = $position['time']; // Current timestamp from 'time' field in the position data

        // Initialize time difference to zero
        $timeDifference = 0;

        // If previous time is set, calculate the time difference (in seconds)
        if ($previousTime !== null) {
            // Calculate the difference between current and previous timestamps
            $currentTimestamp = strtotime($currentTime);
            $previousTimestamp = strtotime($previousTime);
            $timeDifference = $currentTimestamp - $previousTimestamp; // Time difference in seconds
        }

        // Set the previous time for the next iteration
        $previousTime = $currentTime;

        // Accumulate the time based on state
        if ($motion === 'true' && $ignition === 'true') {
            // Device is moving
            $totalMovingHours += $timeDifference;
        } elseif ($ignition === 'true' && $motion === 'false') {
            // Device is idle (engine on but not moving)
            $totalIdleHours += $timeDifference;
        }
    }

    // Convert total moving and idle times from seconds to hours
    $totalMovingTimeHours = $totalMovingHours / 3600;
    $totalIdleTimeHours = $totalIdleHours / 3600;

    // Calculate total stop time based on the remaining time
    $totalStopTimeHours = max(1 - ($totalMovingTimeHours + $totalIdleTimeHours), 0); // Ensure non-negative value

    // Convert total times to formatted strings
    return [
        'totalMovingTime' => formatTime($totalMovingHours),
        'totalIdleTime' => formatTime($totalIdleHours),
        'totalStopTime' => formatTime($totalStopTimeHours * 3600),
    ];
}
...
/*function calculateTotalHours($positions) {
    ...
    foreach ($positions as $position) {
        ...
        $ignition = (string) $xml->ignition; // 'true' or 'false'
        $currentTime = $position['time']; // Current timestamp from 'time' field in the position data
        $speed = $position['speed']; // Current timestamp from 'time' field in the position data
        ...
        if ($motion === 'true' && $ignition === 'true') {
            ...
            // Device is stopped (engine off)
            $totalStopHours += $timeDifference;
        }
    }

    // Convert total time for each state into hours, minutes, and seconds
    return [
        'totalMovingTime' => formatTime($totalMovingHours),
        'totalIdleTime' => formatTime($totalIdleHours),
        'totalStopTime' => formatTime($totalStopHours),
    ];
}*/
...
function fuel_tank_value_day_end($id,$dateSelect)
{
    $dateString = Carbon::parse(Formatter::time()->reverse($dateSelect))->format('Y-m-d');
    $device = \Tobuli\Entities\Device::where('id',$id)->first();
   
    return $device->getFuelDayEnd($dateString)??0;
}
function fuel_tank_value_day_end_date_formate($id,$dateSelect)
{
    $device = \Tobuli\Entities\Device::where('id',$id)->first();
    return $device->getFuelDayEnd($dateSelect)??0;
}
function getEventDetails($id,$date_from,$date_to,$type)
{
     
    $device = \Tobuli\Entities\Device::where('id',$id)->first();
    return $device->last_event()->where('type', $type)->whereDate('time', '>=', $date_from)->whereDate('time', '<=', $date_to)->groupby('time')->get();
}
function getFuelByPositionID($deviceID,$dateTime)
{
    $device = \Tobuli\Entities\Device::where('id',$deviceID)->first();
    return $device->getFuel($dateTime)??0;
}
function getFuelConsumHelper($id,$currentDate,$currentDateEnd)
{

    $device = \Tobuli\Entities\Device::where('id',$id)->first();
    $sensor = $device->getFuelTankSensor('fuel_tank');
    $currDateTime = Formatter::time()->reverse($currentDate);
    $endDateTime = Formatter::time()->reverse($currentDateEnd);
    if($sensor)
    return $device->getFuelConsumByTrip($currDateTime,$endDateTime);
    else
    return 0;
}
function fuel_tank_value($id,$dateSelect)
{
    $dateTime = Formatter::time()->reverse($dateSelect);
    $device = \Tobuli\Entities\Device::where('id',$id)->first();
    return $device->getFuel($dateTime)??0;
}
