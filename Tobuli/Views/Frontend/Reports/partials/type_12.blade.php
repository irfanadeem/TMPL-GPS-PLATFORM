...
@section('content')
        ...
        <div class="panel panel-default">
            ...
            <div id="fuel_theft_count" style="width: 100%; height: 400px;"></div>
                            ...
                            @foreach ($item['table']['rows'] as $row)
                                    @php
                                        ...
                                        if($devices_count_date != $dateString){
                                            if(isset($fuel_theft_data[$dateString])){
                                                $fuel_theft_data[$dateString][0]['fuel'] += (float) str_replace(['liter', 'L'], '', $row['fuel_level_difference']);
                                            }else{
                                                $fuel_theft_data[$dateString][] = [
                                 ...
                                 $fuel_theft_total += (float) str_replace(['liter', 'L'], '', $row['fuel_level_difference']);
                                    @endphp
                                <tr>
            ...
            <table class="table table-striped table-speed">
                <tr>
                    <td> Total Fuel Theft: {{ $fuel_theft_total }} L</td>
                </tr>
            </table>
        ...
        <script>
            $(document).ready(function () {
                var keys = [];
                var dates = {!! json_encode(array_keys($fuel_theft_data)) !!};
                for (var i in dates) {
                    keys.push([i, dates[i]]);
                }

                var dataset = [];
                var data = {!! json_encode($fuel_theft_data) !!};
                var dataIndex = 0;
                ...
                var plot = $.plot($("#fuel_theft_count"), dataset, {
                    yaxis: {
                        font: {
                            size: 12,
                            color: "black",
                    ...
                    legend: {
                        ...
                        container: $('#fuel_theft_count_legends'),
                ...
                $("#fuel_theft_count").bind("plotclick", function (event, pos, item) {
                    if (item) {
