...
class FuelFillingsReport extends DeviceReport
{
    const TYPE_ID = 11;
    ...
    protected function beforeGenerate()
    {
        ...
        $query = $this->getDevicesQuery()->whereHas('events', function($q){
            $q->whereBetween('time', [$this->date_from, $this->date_to])
              ->where('type', Event::TYPE_FUEL_FILL);
    ...
    protected function generateDevice($device)
    {
        ...
        foreach ($events as $event) {
            ...
            // The previous level is calculated by subtracting the fill amount
            $previousLevel = 0;
            $currentLevel = 0;
            ...
            if ($sensorId && $device->sensors) {
                $sensor = $device->sensors->where('id', $sensorId)->first();
                if ($sensor) {
                    // Get the current fuel level from the sensor (this represents the fuel level after filling)
                    $currentLevel = $sensor->value ?? 0;
                    // Calculate previous level by subtracting the fill amount (fuel level before filling)
                    // Ensure previous level is never negative (fuel levels cannot be negative)
                    $previousLevel = max(0, $currentLevel - abs($difference));
                }
            }
